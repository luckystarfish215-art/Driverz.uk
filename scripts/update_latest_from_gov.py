...
def normalise_key(key):
    return "".join(str(key).lower().replace("_", "").replace("-", "").replace(".", "").split())
...
headers = {
    "User-Agent": "Driverz.uk fuel data updater contact: mtamtc76@gmail.com"
}
...
try:
    ...
    write_status("failed", f"Could not download GOV CSV: {exc}")
    raise
...
if not rows:
    write_status("failed", "CSV downloaded but contained no rows.")
    raise SystemExit("CSV downloaded but contained no rows.")
...
for index, row in enumerate(rows, start=1):
    station_id = pick(row, [
        "site_id", "siteid", "site id", "id", "station_id", "station id"
    ], default=str(index))

    station = {
        "id": str(station_id),
        "brand": pick(row, ["brand", "operator", "company"], ""),
        ...
        "e5": to_price(pick(row, ["e5", "unleaded", "super unleaded"])),
        "e10": to_price(pick(row, ["e10", "petrol"])),
        "b7": to_price(pick(row, ["b7", "diesel"])),
        "sdv": to_price(pick(row, ["sdv", "super diesel", "premium diesel"]))
    }
